...
namespace :deploy do
  ...
  task :start_jetty do
    on roles(:app), :in => :sequence, :wait => 5 do
      ...
      execute "mkdir -p #{release_path}/tmp && cd #{release_path}/tmp && (RAILS_ENV=staging /tmp/drs/rvm-auto.sh . curl -L http://librarystaff.neu.edu/DRSzip/new-solr-schema.zip)"
