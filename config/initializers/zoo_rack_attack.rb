...
Rack::Attack.throttle("challenged", limit: 0, period: 60) do |req|
  ...
  if req.user_agent.blank? || !req.user_agent.downcase.include?("bot".downcase)
    if (req.env["HTTP_COOKIE"].blank? || !req.env["HTTP_COOKIE"].include?("cerberus_app_session")) && (req.fullpath.include?("files/") || req.fullpath.include?("datastream_id=content") || facet)
      if !seen
