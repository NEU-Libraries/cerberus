...
RSpec.describe IptcIngestJob, type: :job do
  ...
  describe 'retry_on transient failures' do
    # The in-method rescues handle permanent exceptions (covered in
    ...
    #
    # The block's content (find_by + status guard + update! +
    # maybe_finalize!) is short and self-evident. Asserting the policy
    # is *declared* is enough; poking through ActiveJob's
    # rescue_handlers to invoke the block directly trips the
    # `executions_for` machinery that only works inside a live job run.
