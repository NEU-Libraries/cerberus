...
describe "aggregated statistics job" do
  ...
  describe "run" do
    ...
    it 'gets impressions' do  #for views, downloads, streams
      ...
      @job.run
      @job.files["#{file.pid}"]["view"].should == 1
      @job.files["#{file.pid}"]["download"].should == 1
      @job.files["#{file.pid}"]["stream"].should == 1
    end
    ...
    it 'does not count uncompleted impressions' do
      ...
      @job = AggregatedStatisticsJob.new(date)
      @job.run
      @job.files["#{file.pid}"]["view"].should == 0
    end
    ...
    it 'aggregates impressions up to parent collection' do  #for views, downloads, streams
      ...
      @job = AggregatedStatisticsJob.new(date)
      @job.run
      @job.collections["#{collection.pid}"]["view"].should == 1
      @job.collections["#{collection.pid}"]["download"].should == 1
      @job.collections["#{collection.pid}"]["stream"].should == 1
    end
    ...
    it 'aggregates impressions up to parent community' do  #for views, downloads, streams
      ...
      @job.communities["#{root_community.pid}"]["download"].should == 1
      @job.communities["#{root_community.pid}"]["stream"].should == 1
    end

    it 'creates aggregatedstatistic objects for file, collection, and community' do
      Impression.create(pid: file.pid, session_id: "doop", action: "view", ip_address: "00.00.00", referrer: "direct", user_agent: "RSpec", status: "COMPLETE", public: "true")
      Impression.create(pid: file.pid, session_id: "doop", action: "download", ip_address: "00.00.00", referrer: "direct", user_agent: "RSpec", status: "COMPLETE", public: "true")
      Impression.create(pid: file.pid, session_id: "doop", action: "stream", ip_address: "00.00.00", referrer: "direct", user_agent: "RSpec", status: "COMPLETE", public: "true")
      date = DateTime.now
    ...
    it 'passes through nested collections and communities' do
      ...
      @job = AggregatedStatisticsJob.new(date)
      @job.run
      @job.files["#{nested_file.pid}"]["view"].should == 1
      @job.files["#{nested_file.pid}"]["download"].should == 1
      @job.files["#{nested_file.pid}"]["stream"].should == 1
      @job.collections["#{child_collection.pid}"]["view"].should == 1
      @job.collections["#{child_collection.pid}"]["download"].should == 1
      @job.collections["#{child_collection.pid}"]["stream"].should == 1
      @job.collections["#{test_collection.pid}"]["view"].should == 1
      @job.collections["#{test_collection.pid}"]["download"].should == 1
      @job.collections["#{test_collection.pid}"]["stream"].should == 1
      @job.communities["#{test_community.pid}"]["view"].should == 1
      @job.communities["#{test_community.pid}"]["download"].should == 1
      @job.communities["#{test_community.pid}"]["stream"].should == 1
      @job.communities["#{root_community.pid}"]["view"].should == 1
      @job.communities["#{root_community.pid}"]["download"].should == 1
      @job.communities["#{root_community.pid}"]["stream"].should == 1
      AggregatedStatistic.where(:object_type=>"file").count.should == 1
    ...
    it 'aggregates item_reports up to parent community' do
      ...
      @job = AggregatedStatisticsJob.new(date)
      @job.run
      @job.communities["#{root_community.pid}"]["loader_uploads"].should == 1
    end
    ...
    it 'gets xml_alerts' do #for xml_edits
      ...
      @job = AggregatedStatisticsJob.new(date)
      @job.run
      @job.files["#{file.pid}"]["xml_edits"].should == 2
    end
    ...
    it 'aggregates xml_alerts up to parent collection' do #for xml_edits
      ...
      @job = AggregatedStatisticsJob.new(date)
      @job.run
      @job.collections["#{collection.pid}"]["xml_edits"].should == 2
    end
    ...
    it 'aggregates xml_alerts up to parent community' do #for xml_edits
      ...
      @job = AggregatedStatisticsJob.new(date)
      @job.run
      @job.communities["#{root_community.pid}"]["xml_edits"].should == 2
    end

    it 'creates aggregatedstatistic objects for file, collection, and community with xml_alerts' do
      date = DateTime.now
      xml_alert.first.pid
      XmlAlert.all.count
      @job = AggregatedStatisticsJob.new(date)
  ...
  private
    def get_core_file_size(pid)
      ...
      all_possible_models = [ "ImageSmallFile", "ImageMediumFile", "ImageLargeFile",
                              ...
                              "ZipFile", "AudioFile", "VideoFile", "PageFile", "AudioMasterFile", "VideoMasterFile", "EpubFile", "DatasetFile" ]
