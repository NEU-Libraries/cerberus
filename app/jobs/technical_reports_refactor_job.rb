class TechnicalReportsRefactorJob

  def queue_name
    :technical_reports_refactor
  end

  def run
    # Find all employees that don't have a technical_reports smart collection
    Employee.find(:all).each do |emp|
      ...
      sc.each do |col|
        if col.smart_collection_type == "Technical Reports"
          has_technical_reports = true
        end
      end

      if !has_technical_reports
