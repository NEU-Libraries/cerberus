class AggregatedStatisticsBackFillJob
  ...
  def initialize(date)
    # d = (Time.now - 3.day).midnight - 1.second <- example
    # Cerberus::Application::Queue.push(AggregatedStatisticsBackFillJob.new(d.to_datetime)) <- example
  ...
  def run
    ...
    if (date > (Impression.order('created_at asc').limit(1).first.created_at).to_datetime)
