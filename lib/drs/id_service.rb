...
module Drs
  module IdService
    ...
    def self.noid_template
      Cerberus::Application.config.noid_template
    end

    @minter = ::Noid::Minter.new(:template => noid_template)
    @pid = $$
    @namespace = Cerberus::Application.config.id_namespace
    @semaphore = Mutex.new
    ...
    def self.next_id
      pid = ''
      File.open(Cerberus::Application.config.minter_statefile, File::RDWR|File::CREAT, 0644) do |f|
