...
`#call` product. The resolver is instantiated once per render and read piecemeal
— `SetResolver#contents_fqs` for the controller to layer onto a builder seeded
with the live search state (mirroring `CatalogController#find_children`), then
the per-chip counts, provenance lookups, and aside-zone documents the Set page
renders around the results. Every Solr round-trip is memoized on the instance.
...
`:direct` for an individually added Work, otherwise the noid of the first
included collection whose subtree covers one of the document's membership edges.
...
fetch, and capped at `MAX_EXPORT_ROWS` as a coarse runaway guard until the
...
"just the noid", so the check read nil on every document and withheld nothing.
...
controller, not an action on `SetsController`, because
`ActionController::Live` streams *every* action in its controller — the same
reason `DownloadsController` is its own thing. It subclasses `CatalogController`
so it inherits the `GatedSearchService` and `search_service_context`: the
contents resolve through the identical gated search the Set show page uses.
...
from the file and the same file inside the ZIP, which defeats the only thing
View-as is for.

## Bulk actions over a Set's Works

`SetBulkActions` holds the Set edit page's two bulk actions — privatize, and
apply the Set's derivative-access policy — plus the Sentinel authoring that
feeds one of them.
...
or the devolved tier, via `bulk_operator?` — and `require_bulk_operator` is
...
anyone curated them in. The ceiling that matters here is each Work's own, which
is per-Work and therefore applied at sweep time — `SetSentinelApplyJob` clamps —
rather than at authoring time. Monotonicity still holds, because that is a
...
but it is the same rule with the other branch removed: that helper hands the full
list to an admin or devolved admin and falls back to personal memberships for
...
which is the Collection tab's behaviour on a *private* collection and is the only
coherent choice for a Set: an omitted tier rides each Work's own visibility at
apply time, so one policy can span a Set that mixes public and restricted Works.
...
the Works it denotes, apply a per-Work change, and tell whoever asked for it what
happened.
...
errors abort and which are collected, how a truncated walk is disclosed — is the
same, and needs to stay the same: both are operator actions over content someone
...
Failures are named rather than counted because in both sweeps a Work that did not
change is a Work still carrying the access the operator meant to take away — the
one outcome they must not have to discover for themselves.
...
to touch in one click, and the caller reports the truncation rather than running
...
`total_pages` trusts the pagination when Atlas sends it, so the walk stops on the
last page instead of paying one empty request to discover the end.
